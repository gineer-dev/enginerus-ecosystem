...
import { Edit3, RotateCcw, Send, ToggleLeft, ToggleRight } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { resetInternalUserPassword, resendInternalCredentials, updateInternalAccountStatus } from "@/services/actions";
...
export function InternalUsersTable({ rows }: { rows: InternalUserRow[] }) {
  return (
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          <thead className="bg-muted text-xs uppercase tracking-[0.12em] text-muted-foreground">
            <tr>
              {["Full Name", "Email", "Mobile", "Username", "Role", "Status", "First Login", "Created", "Last Login", "Actions"].map((heading) => (
            ...
            {rows.length ? (
              rows.map((row) => (
                <tr key={row.id} className="border-t border-border/70 align-top hover:bg-muted/50">
                  <td className="whitespace-nowrap px-4 py-3 font-semibold">{row.full_name}</td>
                  ...
                  <td className="whitespace-nowrap px-4 py-3"><Badge className="border-sky-200 bg-sky-50 text-sky-700">{row.role_name}</Badge></td>
                  ...
                  <td className="min-w-[25rem] px-4 py-3">
                    <div className="flex flex-wrap gap-2">
                      <Link href={`/dashboard/account-management/internal-users/${row.id}/edit`}>
                        <Button variant="outline" className="h-9 px-3 text-xs"><Edit3 className="h-4 w-4" /> Edit</Button>
                      </Link>
                      <form action={updateInternalAccountStatus}>
                      ...
                      <form action={resetInternalUserPassword}>
                        <input type="hidden" name="id" value={row.id} />
                        <Button variant="outline" className="h-9 px-3 text-xs"><RotateCcw className="h-4 w-4" /> Reset</Button>
                      </form>
                      <form action={resendInternalCredentials}>
                        <input type="hidden" name="id" value={row.id} />
                        <Button variant="outline" className="h-9 px-3 text-xs"><Send className="h-4 w-4" /> Resend</Button>
                      </form>
                    </div>
                  </td>
                </tr>
              ...
              <tr>
                <td colSpan={10} className="px-4 py-12 text-center text-muted-foreground">No internal users yet.</td>
              </tr>
            )}
          </tbody>
        </table>
